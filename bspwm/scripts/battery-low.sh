...
export DISPLAY=:0
export DBUS_SESSION_BUS_ADDRESS="unix:path=/tmp/dbus-wHv9XOtk5D"
...
# If the battery is low and is not charging (and has not shown notification yet)
    ...
    notify-send "Low Battery" "${BATTERY_LEVEL}% of battery remaining." -i ~/.local/share/icons/dunst/alert.png -u critical -r 9991
    touch $EMPTY_FILE
fi
